 ...
 */
struct signsky_pool *
signsky_pool_init(size_t elm, size_t len)
{
	struct signsky_pool	*pool;
	struct entry		*entry;
	size_t			total, idx;
	...
	pool = signsky_alloc_shared(total, NULL);

	memset(pool, 0, sizeof(*pool));
	pool->len = len;

	signsky_ring_init(&pool->queue, elm);
	...
	for (idx = 0; idx < elm; idx++) {
		...
		if (signsky_ring_queue(&pool->queue, entry) == -1)
 ...
 */
void *
signsky_pool_get(struct signsky_pool *pool)
{
	struct entry		*entry;

	PRECOND(pool != NULL);

	if ((entry = signsky_ring_dequeue(&pool->queue)) == NULL)
		return (NULL);

	if (!signsky_atomic_cas_simple(&entry->free, 1, 0))
 ...
 */
void
signsky_pool_put(struct signsky_pool *pool, void *ptr)
{
	uintptr_t		uptr;
	struct entry		*entry;
	...
	if (!signsky_atomic_cas_simple(&entry->free, 0, 1))
		fatal("failed to mark %p as free", (void *)entry);

	if (signsky_ring_queue(&pool->queue, (void *)uptr) == -1)
